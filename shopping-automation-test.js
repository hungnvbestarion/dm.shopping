...
(async function runShoppingAppTests() {
  ...
  try {
    ...
    try {
      const productListTab = await driver.findElement(By.xpath("//button[contains(., 'Product List')]"));
    ...
    try {
      const productMgmtTab = await driver.findElement(By.xpath("//button[contains(., 'Product Management')]"));
    ...
    try {
      ...
      const cartButton = await driver.findElement(By.css('button.p-overlay-badge, button:has(.pi-shopping-cart)'));
      logTest('Cart Icon Found', true);
      
      // Test: Open Cart Drawer
      await cartButton.click();
      await driver.sleep(1000); // Wait for animation
      const drawer = await driver.findElement(By.css('.p-drawer, .p-sidebar, [role="complementary"]'));
    ...
    try {
      const searchBar = await driver.findElement(By.css('input[placeholder*="search"], input[placeholder*="Search"], [class*="search"] input'));
    ...
    try {
      ...
      const products = await driver.findElements(By.css('.bg-white.rounded-lg.shadow-sm, .product-card'));
      ...
      logTest('Products Displayed', products.length > 0, `Found ${products.length} potential product elements`);
    } catch (e) {
    ...
    try {
      const productNames = await driver.findElements(By.xpath("//*[contains(@class, 'product') or contains(@class, 'title') or contains(@class, 'name')]"));
      logTest('Product Information Visible', productNames.length > 0, `Found ${productNames.length} product element(s)`);
    } catch (e) {
    ...
    try {
      const pagination = await driver.findElements(By.css('.p-paginator, .paginator, [class*="paginator"], [class*="pagination"]'));
    ...
    try {
      const searchInputs = await driver.findElements(By.css('input[type="text"], input[placeholder*="search"], input[placeholder*="Search"]'));
    ...
    try {
      const addButtons = await driver.findElements(By.xpath("//button[contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'add')] | //button[contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'new')]"));
    ...
    // Test: Edit/Delete buttons on products
    try {
      // Wait for table rows to load
      await driver.wait(until.elementLocated(By.css('tr')), 5000);
      
      // Look for "Edit" text which we added to the buttons
      const editSpans = await driver.findElements(By.xpath("//span[contains(text(), 'Edit')]"));
      if (editSpans.length > 0) {
        logTest('Product Action Buttons', true, `Found ${editSpans.length} edit label(s)`);
      } else {
        // Fallback: look for icons
        const iconButtons = await driver.findElements(By.css('.fa-pencil-square-o, .fa-trash-o'));
        if (iconButtons.length > 0) {
             logTest('Product Action Buttons', true, `Found ${iconButtons.length} action icon(s)`);
        } else {
             logTest('Product Action Buttons', false, 'No action buttons or labels found');
        }
      }
    } catch (e) {
      logTest('Product Action Buttons', false, e.message);
    }
    ...
    try {
      ...
      logTest('Page Content Loaded', bodyText && bodyText.length > 50, `Content length: ${bodyText.length} chars`);
    } catch (e) {
    ...
    try {
      const errorElements = await driver.findElements(By.xpath("//*[contains(@class, 'error') or contains(@class, 'alert') or contains(@class, 'warning')]"));
    ...
    try {
      ...
      logTest('Page Fully Loaded', loadingElements.length === 0, `Loading elements: ${loadingElements.length}`);
    } catch (e) {
