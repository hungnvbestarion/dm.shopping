  ...
  <div class="card">
    <MegaMenu :model="items" class="p-4 bg-surface-0" style="border-radius: 3rem">
      ...
      <template #item="{ item }">
        <a
          v-if="item.root"
          class="flex items-center cursor-pointer px-4 py-2 overflow-hidden relative font-medium uppercase"
          style="border-radius: 2rem"
        >
          <span>{{ item.label }}</span>
        </a>
        <a v-else-if="!item.image" class="flex items-center p-2 cursor-pointer gap-2">
          <span
        ...
        <div v-else class="flex flex-col items-start gap-4 p-2">
          ...
          <Button :label="item.label as string" variant="outlined" />
        </div>
      </template>
    </MegaMenu>
  </div>
